...
function initGame() {
    ...
    gameBoard.addEventListener('animationend', event => {
        if (event.target.hasAttribute('data-animation')) {
...
function initModals() {
    ...
    addFavoriteBtn.addEventListener('click', () => {
        addToFavorites();
        addFavoriteBtn.textContent = 'Өстәлгән';
...
function loadStatistics() {
    if (!localStorage.getItem('victories_count')) {
        localStorage.setItem('victories_count', '0');
    }
    if (!localStorage.getItem('loses_count')) {
        localStorage.setItem('loses_count', '0');
    }
    if (!localStorage.getItem('streak_count')) {
        localStorage.setItem('streak_count', '0');
    }
    if (!localStorage.getItem('max_streak')) {
        localStorage.setItem('max_streak', '0');
    }
    if (!localStorage.getItem('favorite_words')) {
        localStorage.setItem('favorite_words', '');
    }
}

function showStatistics() {
    const victories = parseInt(localStorage.getItem('victories_count'));
    const loses = parseInt(localStorage.getItem('loses_count'));
    const streak = parseInt(localStorage.getItem('streak_count'));
    const maxStreak = parseInt(localStorage.getItem('max_streak'));

    const totalGames = victories + loses;
    ...
    document.getElementById('word-display').textContent = `${WORD_OF_THE_DAY}`;
}

function addToFavorites() {
    const favoriteWords = localStorage.getItem('favorite_words');
    const newWord = `${WORD_OF_THE_DAY}`;

    if (favoriteWords.includes(newWord)) return;

    localStorage.setItem('favorite_words',
        favoriteWords ? `${favoriteWords},${newWord}` : newWord);
}

function showMessage(message) {
    const toast = document.createElement('div');
    toast.className = 'wordle-message';
...
function checkGuess(guess, word) {
    ...
    wordLetters.forEach((letter, index) => {
        if (guessLetters[index] === letter) {
            currentRow.querySelector(`li:nth-child(${index + 1})`)
                .setAttribute('data-status', 'correct');
    ...
    remainingWordLetters.forEach((letter, index) => {
        ...
        const guessIndex = remainingGuessLetters.indexOf(letter);
        if (guessIndex !== -1) {
            const column = currentRow
                .querySelector(`li:nth-child(${guessIndex + 1})`);

            if (column.getAttribute('data-status') !== 'correct') {
                column.setAttribute('data-status', 'present');
            }

            const keyboardKey = document.querySelector(`[data-key='${letter}']`);

            if (keyboardKey.getAttribute('data-status') !== 'correct') {
                keyboardKey.setAttribute('data-status', 'present');
            }

            remainingGuessLetters[guessIndex] = false;
        }
    });
    ...
    if (history.length === MAX_NUMBER_OF_ATTEMPTS) {
        showMessage(`Кызганыч, сез сүз тапмадыгыз. Дөрес җавап: ${word}`);
...
function updateStatistics(isWin) {
    let victories = parseInt(localStorage.getItem('victories_count'));
    let loses = parseInt(localStorage.getItem('loses_count'));
    let streak = parseInt(localStorage.getItem('streak_count'));
    let maxStreak = parseInt(localStorage.getItem('max_streak'));
    ...
    localStorage.setItem('victories_count', victories.toString());
    localStorage.setItem('loses_count', loses.toString());
    localStorage.setItem('streak_count', streak.toString());
    localStorage.setItem('max_streak', maxStreak.toString());
}
...
function generateBoard(board, rows = 6, columns = 5, keys = [], keyboard = false) {
